...
# 標記檔的path
xmlFolder = "trashbin_project\labels"
# 圖片檔的path
imgFolder = "trashbin_project\images"
# 要取出的標記名稱(class name)
labelName = "normal_bin"
...
def saveROI(roiSavePath, imgFolder, xmlFilepath, labelGrep="", generateNeg=False):
    ...
    for i in range(0, len(labelName)):
        if (labelGrep == "" or labelGrep == labelName[i]):
            ...
            roiFile = os.path.join(
                roiSavePath, xml_filename + '_' + str(countLabels)+"."+imageKeepType)
    ...
    if (generateNegativeSource == True):
        negFile = os.path.join(negOutput, xml_filename +
                               '_' + str(countLabels)+"."+imageKeepType)
...
for file in os.listdir(xmlFolder):
    ...
    if (file_extension == ".xml"):
        ...
        saveROI(saveROIsPath, imgFolder, xmlfile,
                labelName, generateNegativeSource)
...
with open(positiveDesc_file, 'a') as the_file:
    for file in os.listdir(saveROIsPath):
        ...
        if (file_extension.lower() == ".jpg" or file_extension.lower() == ".png" or file_extension.lower() == ".jpeg"):
            ...
            the_file.write('positives/'+file + '  1  0 0 ' +
                           str(sizeimg[1]) + ' ' + str(sizeimg[0]) + '\n')
